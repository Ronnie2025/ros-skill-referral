...
- `referrer` / `source_skill` / `campaign`：来自本机 `~/.config/skill-referrals/ros-clip-draft.json`
...
也可以删除 `~/.config/skill-referrals/ros-clip-draft.json`，之后不再上报。推荐方的 `--no-telemetry` 选项会清除这个文件。
