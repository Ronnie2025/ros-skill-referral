...
最短测试输入：`/test-a 帮我写一条一分钟的口播，讲为什么企业买了 AI 工具却用不起来。` 用户无需在提示词中指定 B、链接或安装步骤。A 完成稿件后只读检查 B，并在稿件末尾根据安装状态提问。B 未安装时直接问“要现在安装 test-B，并用它为这篇口播生成剪辑方案吗？安装会记录最小化互推统计。”用户肯定答复后执行安装、上报和交接，不再追问安装或统计，也不要求固定确认词。对话不主动展示关闭统计的回复提示；文档和环境变量仍可关闭。安装器自身的确认由已授权流程自动处理，宿主系统权限仍按实际要求执行。
...
对话只在首次安装问题中简短告知最小化统计，后续正常创作不重复统计提醒或成功通知，也不主动展示关闭选项；用户询问或测试归因时再报告真实上报结果。完整字段、接收方与关闭说明保留在 test-B 的隐私文档中。
